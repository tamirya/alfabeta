...
function business_one_page_save_sidebar_layout( $post_id ) { 
    global $business_one_page_sidebar_layout, $post; 
    ...
    foreach( $business_one_page_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, 'business_one_page_sidebar_layout', true ); 
        $new = sanitize_text_field( $_POST['business_one_page_sidebar_layout'] );
        if ( $new && $new != $old ){  
            update_post_meta( $post_id, 'business_one_page_sidebar_layout', $new );  
        } elseif ('' == $new && $old ){  
            delete_post_meta( $post_id, 'business_one_page_sidebar_layout', $old );  
        } 
     } // end foreach   
